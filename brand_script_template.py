"""Template for brand-specific Shopify full-catalog scrapers.

The enriched data structure emitted by scrape_shopify_catalog:
- images_list : list of ALL product image URLs (not just the first)
- size_details: list of {"size": <name>, "available": <bool>} — one entry per
  size from the variants array; out-of-stock sizes are marked available=False
- color       : extracted from options, tags or body description
- fabric      : extracted from options, tags or body description

Copy this file into a brand folder, set the constants, and run it.
"""
import os
import sys
...
OUTPUT_NAME = "example_products.json"
BRAND = "Example"
